...
const faqItems = [
  ...
  },
  {
    ...
    answer:
      'Стоимость химчистки шубы из норки начинается от 10 500 рублей. Точная цена зависит от длины изделия, степени загрязнения и необходимости дополнительных процедур (антимольная обработка, восстановление блеска). Мастер назовёт стоимость после осмотра.',
  },
  {
  ...
  {
    question: 'Выводите ли вы пятна с меха — кофе, косметика, жир?',
    answer:
      'Да. Локальные пятна — кофе, косметика, следы парфюма, жирные пятна у карманов и воротника — выводим щадящими составами, подобранными под конкретный тип и цвет меха. Главное — не пытайтесь оттереть пятно дома: втирание загоняет загрязнение вглубь ворса. Привезите шубу на бесплатный осмотр — мастер скажет сразу, что можно сделать и сколько это будет стоить.',
  },
  {
    question: 'Светлый мех пожелтел — это можно исправить?',
    answer:
      'Пожелтение светлого меха — естественный процесс окисления ворса, одной химчисткой оно снимается лишь частично. Полностью вернуть светлым шубам ровный тон помогает тонирование — эта услуга есть у нас в разделе окрашивания меха. На осмотре мастер честно скажет, что даст лучший результат в вашем случае: чистка, тонирование или их сочетание, — и назовёт итоговую цену до начала работ.',
  },
  {
    question: 'Почему обычная химчистка не берёт мех, а вы берёте?',
    answer:
      'Обычные химчистки чистят вещи в машинах с агрессивными растворителями — для меха это губительно: мездра (кожаная основа) дубеет, ворс тускнеет и ломается. Мы — меховое ателье: чистим мех по щадящей технологии, подбирая метод и составы под каждое изделие, с ручной обработкой деликатных зон. Именно поэтому фабрики и ателье чистят мех сами, а не отправляют его в обычную химчистку.',
  },
]
