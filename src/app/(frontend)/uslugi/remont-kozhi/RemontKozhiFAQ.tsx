...
const faqItems = [
  ...
  },
  {
    ...
    answer:
      'Да, мы выполняем полное и локальное окрашивание кожаных изделий. Подбираем точный оттенок, используем профессиональные красители, которые не трескаются и сохраняют мягкость кожи. Окрашивание кожаной куртки — от 15 000 ₽. Ателье Mary Belle работает с 1870 года и накопило огромный опыт в реставрации кожи.',
  },
  {
  ...
  {
    question: 'Почему замена молнии стоит от 10 000 ₽?',
    answer:
      'Потому что молния в кожаной куртке не пришивается поверх — изделие частично разбирается: распарывается подкладка и борта, старая молния демонтируется, новая вшивается по технологии, затем изделие собирается заново. Это несколько часов работы мастера по коже плюс качественная фурнитура (YKK или аналог), которая переживёт саму куртку. Дешёвая «замена за час» в будке у метро обычно означает пришитую поверх молнию, которая перекосит борт.',
  },
  {
    question: 'Можно ли отремонтировать прожог на дублёнке или коже?',
    answer:
      'Да, прожоги — рабочая для нас задача. В зависимости от размера повреждения мастер подбирает способ: аккуратная заплата из материала в тон, вставка с перекрытием повреждённого участка или локальная реставрация с подкраской. На рукавах и видных местах подбираем материал особенно тщательно, чтобы ремонт не бросался в глаза. Привозите на бесплатный осмотр — скажем честно, каким будет результат.',
  },
]
